...
"""<--PATTERN FACTORY METHOD VERSION 2.0 -> GameBoard2.0-->"""
...
if sys.platform.startswith('win'):
    ...
    def console(char: str, background: str):
...
def create_piece(kind: str, color: str) -> str:
    if kind == 'draught':
        return eval(f"{color.title()}{kind.title()}()")
    return eval(f"{color.title()}Chess{kind.title()}()")
    # Using eval() is risky
...
class {name}(Piece):
    ...
    def __new__(Class):
        return super().__new__(Class, "{char}")""")
    # Using exec() is risky
