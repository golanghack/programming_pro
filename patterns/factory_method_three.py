...
"""<--PATTERN FACTORY METHOD VERSION 3.0 -> GameBoard3.0-->"""
...
def main():
    ...
    if sys.platform.startswith('win'):
        filename = os.path.join(tempfile.gettempdir(), 'gamneboard.txt')
...
class ChessBoard(AbstractBoard):
    ...
    def populate_board(self):
        for row, color in ((0, BLACK), (7, WHITE)):
            for columns, kind in(((0, 7), ROOK), ((1, 6), KNIGHT), ((2, 5), BISHOP), ((3, ), QUEEN),
                                 ((4,), KING)):
