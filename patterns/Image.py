...
class Image:
    ...
    def __init__(self, 
        ...
        assert (width is not None and (height is not None or pixels is not None) or (filename is not None))
    ...
    @classmethod
    def create(Class, width: float, height: float, background: str=None):
    ...
    def load(self, filename: str) -> None:
        ...
        if load is not None:
            ...
            raise Error(f'cannot load files or {suffix} format')
    ...
    def save(self, filename: str=None) -> None:
        ...
        if not filename:
            raise Error('cant save without a filename')
        suffix = os.path.splitext(filename)[1].lower()
        ...
        if save is not None:
            ...
            raise Error(f'cannot save files or {suffix} format')
        
    def pixel(self,int x,int y):
        """Returns the color at the given pixel as an ARGB int
    ...
    def rectangle(self, int x0, int y0, int x1, int y1, outline=None, fill=None):
    ...
    def ellipse(self, int x0, int y0, int x1, int y1, outline=None, fill=None):
