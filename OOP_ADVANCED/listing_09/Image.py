...
""" 
This module provides the Image which holds (x, y, color) triples 
and a background color to provide a kind of the sparse-array 
representions of an image. A method to export image in XPM 
...
class Image:
    ...
    def __init__(self, 
        ...
        """An image represented as HTML-style color 
        values (color names or hex strings) at (x, y)
        coordinates with any unspecified points assumed to be the background
        """ 
    ...
    def save(self, filename: str=None):
        """Saves the current image, or thye one specified by filename 
        ...
        try:
            ...
            file_header = open(self.filename, 'wb')
            pickle.dump(data, file_header, pickle.HIGHEST_PROTOCOL)
    ...
    def load(self, filename: str=None):
        ...
        if filename is specified the interal filename is set to it
        """ 
        ...
        file_header = None
        try:
            file_header = open(self.filename, 'rb')
            data = pickle.load(file_header)
    ...
    def export(self, filename: str):
        """Exports the iamge to the specified filename""" 
    ...
    def __export_xpm(self, filename: str):
        """Export the image as an XPM file if less than 8930 colors are used"""
        ...
        file_header = None
        try:
            file_header = open(filename, 'w', encoding='ascii')
            file_header.write('/* XPM */\n')
            file_header.write("static char *{0}[] = {{\n".format(name))
            file_header.write("/* columns rows colors chars-per-pixel */\n")
            file_header.write('"{0.width} {0.height} {1} {2}",\n'.format(
                     self, count, len(chars[0])))
            char_for_colour = {}

            for color in self.__colors:
                char = chars.pop() 
                file_header.write('"{char} c {color}", \n'.format(**locals()))
                char_for_colour[color] = char 
            file_header.write('/* pixels */\n')
