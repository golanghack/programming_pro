...
if hasattr(xml.etree.ElementTree, 'ParseError'): #python version 3.2
    class Worker(threading.Thread):
        ...
        def process(self, filename: str) -> None:
            ...
            except xml.etree.ElementTree.ParseError as err: # python version 3.2
                message = 'has the following error:\n   ' + str(err)
            except xml.parsers.expat.ExpatError as err:
                message = 'has the following error:\n    ' + str(err)
            except EnvironmentError as err:
                message = 'could not me read:\n    ' + str(err)
            else:
                message = ('uses the following tags:\n    ' + '\n    '.join(sorted(tags, key=str.lower)))
            message = (self.number + filename + ' is an XML format file that ' + message)
