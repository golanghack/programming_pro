...
def indented_list_sort(indented_list: list, indent: str='    '):
    ...
    KEY, ITEM, CHILD = range(3)
    
    def add_entry(level: int, key: int, item: int, child: list):
        if level == 0:
            child.append((key, item, []))  
        else:
            add_entry(level -1, key, item, child[-1][CHILD])
            
    def update_indented_list(entry: list):
        indented_list.append(entry[ITEM])
        for subentry in sorted(entry[CHILD]):
...
def indented_list_sort_local(indented_list: list, indent: str='    '):
    """
    ...
    def add_entry(key: int, item: int, child: list):
        nonlocal level 
        if level == 0:
            child.append((key, item, []))
            ...
            level -= 1
            add_entry(key, item, child[-1][CHILD])
...
if __name__ == '__main__':
    ...
    first_result = indented_list_sort(before)
    second_result = indented_list_sort_local(before)
    ...
    assert first_result == second_result == after
