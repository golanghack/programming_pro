...
class BinaryRecordFile:
    ...
    def __getitem__(self, index: int) -> bytearray:
    ...
    def compact(self, keep_backup: bool=False) -> None:
    ...
    def inplace_compact(self) -> None:
        ...
        while index < lenght:
            ...
            state = self.__fh.read(1)
            if state != _OKAY:
                for next_ in range(index + 1, lenght):
                    self.__seek_to_index(next_)
                    state = self.__fh.read(1)
                    if state == _OKAY:
                        self[index] = self[next_]
                        del self[next_]
                        break
                else:
                    break
