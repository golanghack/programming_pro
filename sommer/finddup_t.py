...
"""<--FIND DUPLICATIONS WITH THREADING-->"""
...
class Worker(threading.Thread):
    ...
    def process(self, size, filenames):
        ...
        for filenames in md5s.values():
            if len(filenames) == 1:
                continue
            self.results_queue.put(f'{self.number} Duplicate files ({size:n} bytes: '
...
def main():
    ...
    if opts.verbose:
        print(f'Creating {opts.count} thread{Util.s(opts.count)}...')
