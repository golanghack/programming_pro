...
INVENTORY_BASE = 'http://127.0.0.1:8081'
FAVORITE_BASE = 'http://127.0.0.1:8082'
CART_BASE = 'http://127.0.0.1:8083'
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if products in pending:
            ...
            return web.json_response(
                {'error': 'Dont connect for service of products'},
                status=504
            )
        elif products in done and products.exception() is None:
            [request.cancel() for request in requests]
            logging.exception('Server Error with connection service products', exc_info=products.exception())
            return web.json_response(
                {'error': 'Error server'},
                status=500
            )
...
async def get_products_with_inventory(session: ClientSession, product_response) -> List[Dict]:
    ...
    inventory_tasks_to_product_id = {
        get_inventory(session, product['product_id']): product['porduct_id']
    ...
    for done_task in inventory_done:
        if done_task.exception() is None:
            ...
            logging.exception(
                f'Not response about product {product_id}', 
                exc_info=inventory_tasks_to_product_id[done_task].exceprion())
