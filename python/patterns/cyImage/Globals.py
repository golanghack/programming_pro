...
CLEAR_ALPHA = 0X00FFFFFF #alpha channnel
MAX_ARGB = 0XFFFFFFFF
MAX_COMPONENT = 0xFF
SOLID = 0xFF000000 #ARGB 

class Error(Exception): pass

def sanitized_name(name: str) -> str:
    """return a name suitable XBM and XPM images"""
...
def create_array(width: float, height: float, background: str=None):
    """returns a numpy array of the correct size and with the given background color"""
...
def argb_for_color(color: str):
    """returns an ARGB quadruple for a color specified as an int or a 
    color name otr an #HHH, #HHHH
    #HHHHHH, #HHHHHHHH RGB str ---in the latter case the alpha channel is set to 0XFF(solid)if
    not specified
    """
    ...
    if not isinstance(color, int) or not (0 <= color <= MAX_ARGB):
...
def color_for_rgb(r, g, b):
    """Returns an int representing the given RGB value -> the alpha channel is set
    to 0xFF (solid)
    """
    
    return color_for_argb(MAX_COMPONENT, r, g, b)

def color_for_name(name: str) -> list:
