...
"""<--RENDER1 -- ADAPTER PATTERN ILLUSTRATED-->"""
...
#option for different version python
...
class Page:
    """Class Page render base"""
    
    def __init__(self, title, renderer):
        if not isinstance(renderer, Renderer):
            raise TypeError(f'expected object of type Rendere, got {type(renderer).__name__}')
        self.title = title
...
def main():
    paragraph_one = MESSAGE.format('plain-text', 'TextRenderer')
    paragraph_two = """This is another short paragraph just so that we can see two paragraphs in action"""
    title = 'Plain Text'
    text_page = Page(title, TextRenderer(22))
    text_page.add_paragraph(paragraph_one)
    text_page.add_paragraph(paragraph_two)
    text_page.render()
    
    print()
    
    paragraph_one = MESSAGE.format('HTML', 'HTMLRenderer')
    title = 'HTML'
    file = sys.stdout
    html_page = Page(title, HtmlRenderer(HtmlWriter(file)))
    html_page.add_paragraph(paragraph_one)
    html_page.add_paragraph(paragraph_two)
    html_page.render()
